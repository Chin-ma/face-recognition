...
def firstScript():
    exec(open(r'sqldata.py').read())

def secondScript():
    exec(open(r'genrate.py').read())

def thirdScript():
    exec(open(r'facetrain.py').read())

buttonFont = font.Font(family='Helvetica', size=16, weight='bold')
fButton = Button(master, text="Start Mask Detection", command=firstScript, font=buttonFont)
fButton.place(x=100, y=25)
sButton = Button(master, text="Add new face", command=acceptData, font=buttonFont)
